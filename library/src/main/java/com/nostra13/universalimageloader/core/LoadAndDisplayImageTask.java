...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
        ...
        try {
            ...
            if (bmp == null || bmp.isRecycled()) {
                ...
                if (bmp != null && options.isCacheInMemory()) {
                    if (imageLoadingInfo.mFileTypeStatus != BitmapDisplayer.FILE_TYPE_GIF_IMAGE) {//gif图片 不加入内存缓存
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {//磁盘缓存存在
            ...
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {//重新下载图片流程
